...
init python: 
    ...
    def trade(seller, buyer, item):
        item_reg = item[0].region
        
        #If buyer is brain is buyer, check for region first
        if isinstance(seller, Gestalt):
            if item_reg in buyer.region:
                seller.drop(item[0])
                seller.current_size-=1
                buyer.take(item[0])
            else:
                renpy.show_screen("inventory_popup", message="FAULT!")
        elif isinstance(seller, Brain_Region):
            #Do not drop memory if GESTALT is full
            if not buyer.current_size == buyer.max_size:
                seller.drop(item[0]) 
                buyer.take(item[0]) 
            else:
...
#THIS IS THE CONSTRUCTOR THAT MEMENTO INVENTORIES USE
